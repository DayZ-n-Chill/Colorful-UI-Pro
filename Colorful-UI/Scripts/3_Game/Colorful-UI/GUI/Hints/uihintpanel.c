...
{	
    ...
    protected int m_SlideShowDelay    = 3000; 
}
...
modded class UiHintPanelLoading extends UiHintPanel
{
	...
	override protected void BuildLayout(Widget parent_widget)
	{
		...
		if (m_RootFrame)
		{
			...
			ImageWidget m_tipIcon   = ImageWidget.Cast(m_RootFrame.FindAnyWidget("hintIcon"));	
			ImageWidget m_tShader   = ImageWidget.Cast(m_RootFrame.FindAnyWidget("TopShader"));
			ImageWidget m_bShader   = ImageWidget.Cast(m_RootFrame.FindAnyWidget("BottomShader"));
